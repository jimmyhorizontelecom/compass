...
function initMonthYearInput() {
    ...
    $(document).on("input", ".monthYearInput", function () {
    ...
    $(document).on("blur", ".monthYearInput", function () {

        let value = $(this).val();

        if (value.length !== 6) {
            toastr.error("Enter MonthYear in MMyyyy format (e.g. 102025)");
            $(this).val("").focus();
            return;
        }

        let month = parseInt(value.substring(0, 2));
        let year = parseInt(value.substring(2, 6));

        if (month < 1 || month > 12) {
            toastr.error("Month must be between 01 and 12");
            $(this).val("").focus();
            return;
        }
