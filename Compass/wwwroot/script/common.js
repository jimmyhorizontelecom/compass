...
$('.decimalUpToThreePoints').on('input', function () {
    let val = this.value;
...
$('.decimalOnly').on('input', function () {
    ...
    if (parts.length === 2) {
        parts[1] = parts[1].substring(0, 2); // <-- changed from 3 to 2
        val = parts[0] + '.' + parts[1];
    }
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (savedHeader && headerElement) {
        var titleSpan = headerElement.querySelector(".title-text");
        if (titleSpan) {
            titleSpan.textContent = savedHeader;
        }
    }
});
